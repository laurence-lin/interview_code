...
from utils import is_leap, get_order_date
...
def cnt_date_diff(start_dt, end_dt):
    ...
    if abs(end_yr - str_yr) > 0:  # If start year and end year is not same
        ...
        if str_m == end_m:
            if end_d > str_d: # calculate day difference if start date != end date
               cnt += (end_d - str_d - 1)
